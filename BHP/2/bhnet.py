...
#A custom python script to replace nc
...
# this runs a command returns the output
def run_command(cmd):
    # trim newline
    cmd = cmd.rstrip()
    ...
    # send the output bck to the client
    return output
...
def client_handler(client_socket):
    ...
    if len(upload_destination):
        ...
        # read in all the bytes and write to out destination
        file_buffer = ""
...
# if we don't listen we are a client, so make it so
def client_sender(buffer):
    ...
    try:
        ...
        # if not we are going to wait for the user to punch something in
        if len(buffer):
...
def usage():
    print("Netcat Replacement written in Python via BlackHat Python")
    print()
